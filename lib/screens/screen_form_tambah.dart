...
class _ScreenFormTambahResepState extends State<ScreenFormTambahResep>
  ...
  void _onReviewPressed() {
    ...
    if (_bahanList.isEmpty || _bahanList.every((b) => b.trim().isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Row(
            children: [
              Icon(Icons.warning, color: Colors.white),
              SizedBox(width: 12),
              Text('Tambahkan minimal 1 bahan'),
            ],
          ),
          backgroundColor: MyThemes.primaryColor,
          behavior: SnackBarBehavior.floating,
    ...
    if (_langkahList.isEmpty || _langkahList.every((l) => l.trim().isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Row(
            children: [
              Icon(Icons.warning, color: Colors.white),
              SizedBox(width: 12),
              Text('Tambahkan minimal 1 langkah'),
            ],
          ),
          backgroundColor: MyThemes.primaryColor,
          behavior: SnackBarBehavior.floating,
    ...
    if (_formKey.currentState!.validate()) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => ScreenReviewResep(resep: resepDTO)),
      );
    }
  }
}
